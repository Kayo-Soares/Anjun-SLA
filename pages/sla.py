"""
Página: Indicador de SLA Operacional — visão geral (estado, backlog, baixas
por hora, antecedência, supervisor, DSP, entregador). A base já foi
carregada e processada no app.py (entrada única) e fica em st.session_state;
esta página só lê e renderiza.
"""
...
mapa_supervisor = st.session_state.get("mapa_supervisor")
...
if backlog > 0:
    ...
    # Donut: se tiver mais status do que a paleta consegue distinguir bem,
    # agrupa o restante (menor volume) em "Outros" só no gráfico — a tabela ao
    # lado continua com o detalhe completo, linha por status, sem agrupar.
    if len(comp) > MAX_FATIAS_DONUT_STATUS:
        comp_donut = comp.iloc[:MAX_FATIAS_DONUT_STATUS].copy()
        qtd_outros = comp.iloc[MAX_FATIAS_DONUT_STATUS:]['qtd'].sum()
        comp_donut = pd.concat([
            comp_donut,
            pd.DataFrame([{'status': 'Outros', 'qtd': qtd_outros, 'status_show': L('Outros', '其他')}])
        ], ignore_index=True)
        cores_donut = PALETA_STATUS[:MAX_FATIAS_DONUT_STATUS] + [COR_OUTROS_STATUS]
    else:
        comp_donut = comp
        cores_donut = PALETA_STATUS[:len(comp_donut)]
    ...
    with col_comp1:
        ...
        if len(comp) > MAX_FATIAS_DONUT_STATUS:
            st.caption(L(
                f"'Outros' agrupa {len(comp) - MAX_FATIAS_DONUT_STATUS} status de menor volume — "
                "detalhe completo na tabela ao lado.",
                f"「其他」汇总了 {len(comp) - MAX_FATIAS_DONUT_STATUS} 个占比较小的状态——完整明细见右侧表格。"
            ))
    with col_comp2:
        cores_status = PALETA_STATUS
        linhas_status = ""
...
render_html(montar_tabela_html(tabela))
...
if not tem_entregador:
    st.info(L(
        "Esta base não traz a coluna de entregador individual — normalmente isso acontece "
        "quando o export vem do relatório resumido (sem detalhe de cadeia completa). "
        "Baixe o export completo do sistema pra habilitar esta seção.",
        "本次数据不包含配送员字段——通常发生在使用简化版导出报表时（不含完整链路明细）。"
        "请从系统下载完整版导出报表以启用此功能。"
    ))
else:
